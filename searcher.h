...
class MultiSearcher {
 ...
 private:
   ...
   */
	...
	/**
   * Function finds some nearest points to the some part of query point 
	 * @param point query point
	 * @param start first dimension of necessary part
	 * @param finish dimension after the last dimension of necessary part
	 * @param centroids set of points where function seeks the nearest
	 * @param k number of centroids to get
	 * @param nearest_subpoints result
   */
	//void GetKNearestSubpoints(const Point& point,
	//                          Dimensions start, Dimensions finish,
	//					                const vector<Point>& centroids, int k,
	//					                vector<pair<Distance, PointId> >* nearest_subpoints) const;
	/**
    ...
    coarse_vocabs_matrices_[coarse_id] = new float[coarse_vocabs_[0].size() * coarse_vocabs_[0][0].size()];
		for(int i = 0; i < coarse_vocabs_[0].size(); ++i) {
		  ...
		  for(int j = 0; j < coarse_vocabs_[0][0].size(); ++j) {
		    coarse_vocabs_matrices_[coarse_id][coarse_vocabs_[0][0].size() * i + j] =
					                     coarse_vocabs_[coarse_id][i][j];
...
//template<class Record, class MetaInfo>
//void
//MultiSearcher<Record, MetaInfo>::GetKNearestSubpoints(const Point& point,
//	                                                    Dimensions start,
//                                                      Dimensions finish,
//						                                          const vector<Point>& points,
//                                                      int k,
//						                                          vector<pair<Distance, PointId> >* nearest_subpoints) const {
//  nearest_subpoints->resize(points.size());
//  for(PointId pid = 0; pid < points.size(); ++pid) {
//		Distance current_distance = Eucldistance(point, points[pid], start, finish);
//		nearest_subpoints->at(pid) = std::make_pair(current_distance, pid);
//	}
//	std::nth_element(nearest_subpoints->begin(), nearest_subpoints->begin() + k, nearest_subpoints->end());
//	nearest_subpoints->resize(k);
//	std::sort(nearest_subpoints->begin(), nearest_subpoints->end());
//}
